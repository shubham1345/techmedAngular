...
@Component({
  selector: 'app-main-dashboard-new-appointment',
  templateUrl: './main-dashboard-new-appointment.component.html',
  styleUrls: ['./main-dashboard-new-appointment.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class MainDashboardNewAppointmentComponent  implements AfterViewInit,OnInit {
  loading = false;
  ...
  ngOnInit(): void {
  // this.GetAppointedPatientListAsync()
  }
...
GetAppointedPatientsReportAsync(){
  ...
  let SortDirection='DESC';
    let SearchValue=(this.doctorForm.value.patientName).trim();
  ...
  this.Svc_dashboardService.GetAppointedPatientsReportAsync(PageNo,this.lastpage,this.configPage.SortColumn,SortDirection,SearchValue,FromDate,ToDate,UserID)
  .subscribe((response: Blob) => {
    this.loading=false;
    const url = URL.createObjectURL(response);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'Appointment.csv'; // Specify the file name and extension
    link.click();
  },
  (error)=>{
    this.loading=false;
...
AppointmentPatientReportResultAsync(){
  ...
  this.Svc_dashboardService.AppointmentPatientReportResultAsync(PageNo,this.lastpage,this.configPage.SortColumn,SortDirection,SearchValue,FromDate,ToDate,UserID)
  .subscribe((response: Blob) => {
    this.loading=false;
    const url = URL.createObjectURL(response);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'Appointment'+' '+
...
filterCount:any=0
  GetAppointedPatientListAsync(){
   this.loading=true
    ...
    this.Svc_dashboardService.GetAppointedPatientListAsync(this.currentPage,this.pageSize,this.configPage.SortColumn,SortDirection,SearchValue,FromDate,ToDate,UserID).subscribe(
      (data:any )=>{
