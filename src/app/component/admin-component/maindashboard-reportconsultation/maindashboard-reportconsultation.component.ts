...
@Component({
  selector: 'app-maindashboard-reportconsultation',
  templateUrl: './maindashboard-reportconsultation.component.html',
  styleUrls: ['./maindashboard-reportconsultation.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class MaindashboardReportconsultationComponent implements AfterViewInit {
  loading = false;
  ...
  constructor(
    ...
    this.dataSource.filterPredicate = ((data, filter) => {
      ...
      const a = !filter.age || data.age.toLowerCase().includes(filter.age);
      const b =
        !filter.districtName ||
  ...
  GetDashboardReportConsultationWithRestrictedAccess(params: any) {
    ...
    if (this.userObjFromToken) {
      this.Svc_dashboardService.GetDashboardReportConsultationWithRestrictedAccess(
        params
      ).subscribe(
        (data: any) => {
          console.log(
            data,
            'this is GetDashboardReportConsultationWithRestrictedAccess data'
          );
          ...
          this.dataSource.filterPredicate = ((data, filter) => {
            ...
            const h =
              ...
              data.mobileNo
                .toLowerCase()
                .includes(filter.mobileNo.toLowerCase());
            const i =
              !filter.opdNo ||
              data.opdNo.toLowerCase().includes(filter.opdNo.toLowerCase());
            const j =
              !filter.specialization ||
              data.specialization
                .toLowerCase()
      ...
      forkJoin([
        ...
        next: (data: any) => {
          ...
          if (this.openPrescriptionData[0].prescriptionFilePath == 'pending') {
            ...
            this.openPrescriptionData[0].prescriptionFilePath ==
            'consultationpending'
          ) {
            Swal.fire({
              title: 'Warning',
            ...
            const storage =
              this.openPrescriptionData[0].storageSource == null
                ? 0
                : this.openPrescriptionData[0].storageSource;
      ...
      return new Promise((resolve, reject) => {
        this.newArrayOfObj = this.dataSourceDownload.map((item, index) => {
          return {
            ...
            'OPD No': item.opdNo,
            'Referral Date': this.datepipe.transform(
              item.reviewDate,
              'dd-MM-yyyy'
            ),
  ...
  service = {
    getData: () =>
      of(this.saveAsExcelFile(this.excelBuffer, this.ReportName)).pipe(
        delay(1500)
      ),
  };
  ...
  exportAsExcelFile(array: any, reportname: any) {
    ...
    request.arrayBuffer().then((buffer) => {
      this.saveAsExcelFile(this.excelBuffer, reportname).subscribe(
        (res: any) => {
  ...
  exportPdf() {
    ...
    this.dataSourceDownload.forEach((e) => {
      ...
      tempObj.push(e.mobileNo);
      tempObj.push(e.opdNo);
      tempObj.push(this.datepipe.transform(e.reviewDate, 'dd-MM-yyyy'));
      tempObj.push(this.datepipe.transform(e.consultDate, 'dd-MM-yyyy'));
      tempObj.push(
        this.datepipe.transform(e.starttime, 'dd-MM-yyyy hh:mm:ss a')
      );
      tempObj.push(this.datepipe.transform(e.endtime, 'dd-MM-yyyy hh:mm:ss a'));
      tempObj.push(e.specialization);
      tempObj.push(e.doctor);
      //tempObj.push(e.prescription);
      prepare.push(tempObj);
