...
@Component({
  selector: 'app-maindashboard-medprescribedphcwise',
  templateUrl: './maindashboard-medprescribedphcwise.component.html',
  styleUrls: ['./maindashboard-medprescribedphcwise.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class MaindashboardMedprescribedphcwiseComponent implements AfterViewInit {
  loading = false;
  ...
  constructor(
    ...
    this.ReportName = 'Medicine Prescribed PHC Wise';
    ...
    // this.GetPrescribedMedicinePHCWiseWithRestrictedAccess({
    //   params,
    // });
    // this.GetPrescribedMedicinePHCWise({
    //   params,
    // });
    //////////////////////////////////////////////////////////////
    this.dataSource.filterPredicate = ((data, filter) => {
      ...
      // const e = !filter.prescribedMedicine || data.prescribedMedicine === filter.prescribedMedicine;
      const e = !filter.prescribedMedicine || data.prescribedMedicine.toLowerCase().includes(filter.prescribedMedicine);
      const f = !filter.cluster || data.cluster.toLowerCase().includes(filter.cluster);
      const g = !filter.phase || data.phase.toLowerCase().includes(filter.phase);
      // const f = !filter.numberOfTimePrescribed || data.numberOfTimePrescribed === filter.numberOfTimePrescribed;
      // const g = !filter.eAushadhiStock || data.eAushadhiStock === filter.eAushadhiStock;
      // const h = !filter.quantityPrescribed || data.quantityPrescribed === filter.quantityPrescribed;
      return  b && c && d && e && f && g ;
  ...
  GetPrescribedMedicinePHCWiseWithRestrictedAccess(params: any) {
    this.loading=true
    this.userObjFromToken = this.svcLocalstorage.GetData(
      environment.AuthTokenKeyLSKey
    );
  // GetPrescribedMedicinePHCWise(params: any) {
  //   this.loading=true
  //   this.userObjFromToken = this.svcLocalstorage.GetData(
  //     environment.AuthTokenKeyLSKey
  //   );
    if (this.userObjFromToken) {
      this.Svc_dashboardService.GetPrescribedMedicinePHCWiseWithRestrictedAccess(params).subscribe(
        (data: any) => {
          this.loading=false;
          console.log(data, 'this is GetPrescribedMedicinePHCWiseWithRestrictedAccess data');
          ...
          console.log(
            this.ELEMENT_DATA,
            'this is GetPrescribedMedicinePHCWiseWithRestrictedAccess data'
          );
          ...
          this.dataSource.filterPredicate = ((data, filter) => {
            ...
            // const e = !filter.prescribedMedicine || data.prescribedMedicine === filter.prescribedMedicine.toLowerCase();
            const e = !filter.prescribedMedicine || data.prescribedMedicine.toLowerCase().includes(filter.prescribedMedicine.toLowerCase());
            const f = !filter.cluster || data.cluster.toLowerCase().includes(filter.cluster.toLowerCase());
            const g = !filter.phase || data.phase.toLowerCase().includes(filter.phase.toLowerCase());
      ...
      // this.Svc_dashboardService.GetPrescribedMedicinePHCWise(params).subscribe(
      //   (data: any) => {
      //     this.loading=false;
      //     console.log(data, 'this is GetPrescribedMedicinePHCWise data');
      //     this.ELEMENT_DATA = data;
      //     this.HeaderData = data;
      //     this.dataSourceDownload = this.ELEMENT_DATA;
      //     this.isValid=true;
      //     console.log(
      //       this.ELEMENT_DATA,
      //       'this is GetPrescribedMedicinePHCWise data'
      //     );
      //     this.dataSource = new MatTableDataSource<PeriodicElement>(
      //       this.ELEMENT_DATA
      //     );
  ...
  public Download(value: any) {
    ...
    if (value == 'Excel') {
      ...
      const newArrayOfObj = this.dataSourceDownload.map(item => {
        return {
         'SL No.':item.srNo ,'Cluster':item.cluster,'District':item.districtName,'Block':item.blockName,'PHC':item.phcName,'Phase':item.phase,'Medicine':item.prescribedMedicine,'No. Of Time Prescribed':item.numberOfTimePrescribed,'E-Aushadhi Stock':item.eAushadhiStock,
        'Quantity Prescribed':item.quantityPrescribed};
      });
  ...
  downloadPdf() {
    ...
    this.dataSourceDownload.forEach((e) => {
      ...
      tempObj.push(e.prescribedMedicine);
      tempObj.push(e.numberOfTimePrescribed);
      tempObj.push(e.eAushadhiStock);
      tempObj.push(e.quantityPrescribed);
...
export interface PeriodicElement {
  ...
  numberOfTimePrescribed: number;
  eAushadhiStock: number;
