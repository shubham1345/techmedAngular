...
@Component({
  selector: 'app-maindashboard-diagnosticprescribedphcwise',
  templateUrl: './maindashboard-diagnosticprescribedphcwise.component.html',
  styleUrls: ['./maindashboard-diagnosticprescribedphcwise.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class MaindashboardDiagnosticprescribedphcwiseComponent implements AfterViewInit {
  loading = false;
  ...
  constructor(
    ...
    this.ReportName = 'Diagnostic Prescribed PHC wise';
    ...
    // this.GetDashboardDiagnosticPrescribedPHCWiseWithRestrictedAccess({
    //   params,
    // });
    // this.GetDashboardDiagnosticPrescribedPHCWise({
    //   params,
    // });
    //////////////////////////////////////////////////////////////
    this.dataSource.filterPredicate = ((data, filter) => {
      const a = !filter.noOfTimePrescribed || data.noOfTimePrescribed === filter.noOfTimePrescribed;
      const b =
        !filter.districtName ||
      ...
      const e = !filter.diagnostic || data.diagnostic === filter.diagnostic;
      const f = !filter.cluster || data.cluster === filter.cluster;
      const g = !filter.phase || data.phase === filter.phase;
      return a && b && c && d && f && g;
  ...
  GetDashboardDiagnosticPrescribedPHCWiseWithRestrictedAccess(params: any) {
    this.loading=true;
    this.userObjFromToken = this.svcLocalstorage.GetData(
      environment.AuthTokenKeyLSKey
    );
  // GetDashboardDiagnosticPrescribedPHCWise(params: any) {
  //   this.loading=true;
  //   this.userObjFromToken = this.svcLocalstorage.GetData(
  //     environment.AuthTokenKeyLSKey
  //   );
    if (this.userObjFromToken) {
      this.Svc_dashboardService.GetDashboardDiagnosticPrescribedPHCWiseWithRestrictedAccess(params).subscribe(
        (data: any) => {
          this.loading=false;
          console.log(data, 'this is GetDashboardDiagnosticPrescribedPHCWiseWithRestrictedAccess data');
          ...
          console.log(
            this.ELEMENT_DATA,
            'this is GetDashboardDiagnosticPrescribedPHCWiseWithRestrictedAccess data'
          );
          ...
          this.dataSource.filterPredicate = ((data, filter) => {
            ...
            const e = !filter.diagnostic ||  data.diagnostic.toLowerCase().includes(filter.diagnostic.toLowerCase());
      ...
      // this.Svc_dashboardService.GetDashboardDiagnosticPrescribedPHCWise(params).subscribe(
      //   (data: any) => {
      //     this.loading=false;
      //     console.log(data, 'this is GetDashboardDiagnosticPrescribedPHCWise data');
      //     this.ELEMENT_DATA = data;
      //     this.HeaderData = data;
      //     this.dataSourceDownload = this.ELEMENT_DATA;
      //     this.isValid=true;
      //     console.log(
      //       this.ELEMENT_DATA,
      //       'this is GetDashboardDiagnosticPrescribedPHCWise data'
      //     );
      //     this.dataSource = new MatTableDataSource<PeriodicElement>(
      //       this.ELEMENT_DATA
      //     );
  ...
  public Download(value: any) {
    ...
    if (value == 'Excel') {
      ...
      const newArrayOfObj = this.dataSourceDownload.map(item => {
        return {
         'SL No.':item.srNo ,'Cluster':item.cluster ,'District':item.districtName,'Block':item.blockName,'PHC':item.phcName,'Phase':item.phase ,'Diagnostic':item.diagnostic,'No. Of Time Prescribed':item.noOfTimePrescribed};
      });
  ...
  downloadPdf() {
    ...
    this.dataSourceDownload.forEach((e) => {
      ...
      tempObj.push(e.diagnostic);
      tempObj.push(e.noOfTimePrescribed);
