...
@Component({
  selector: 'app-maindashboard-diagnosis',
  templateUrl: './maindashboard-diagnosis.component.html',
  styleUrls: ['./maindashboard-diagnosis.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class MaindashboardDiagnosisComponent implements AfterViewInit {
  ...
  constructor(
    ...
    this.ReportName = 'Diagnosis Speciality Wise';
    ...
    if (this.specializationID == '1') {
      this.ReportName = 'Diagnosis General Medicine';
    }
    if (this.specializationID == '2') {
      this.ReportName = 'Diagnosis Obstetrics and Gyne';
    }
    if (this.specializationID == '3') {
      this.ReportName = 'Diagnosis Pediatrics';
    }
    ...
    params = params.append('fromDate', this.datepipe.transform(new Date(), 'yyyy-MM-dd'));
      ...
      params = params.append('Specialityid', this.specializationID);
    // this.GetDashboardDignosisSpecialityWiseWithRestrictedAccess({
    //   params
    // });
    // this.GetDashboardDignosisSpecialityWise({
    //   params
    // });
    //////////////////////////////////////////////////////////////
    this.dataSource.filterPredicate = ((data, filter) => {
      const a = !filter.occurrence || data.occurrence === filter.occurrence;
      const b = !filter.districtName ||data.districtName.toLowerCase().includes(filter.districtName);
      ...
      const e = !filter.disease || data.disease === filter.disease;
  ...
  GetDashboardDignosisSpecialityWiseWithRestrictedAccess(params: any) {
    this.userObjFromToken = this.svcLocalstorage.GetData(
      environment.AuthTokenKeyLSKey
    );
  // GetDashboardDignosisSpecialityWise(params: any) {
  //   this.userObjFromToken = this.svcLocalstorage.GetData(
  //     environment.AuthTokenKeyLSKey
  //   );
    if (this.userObjFromToken) {
      this.loading=true
      this.Svc_dashboardService.GetDashboardDignosisSpecialityWiseWithRestrictedAccess(params).subscribe(
        (data: any) => {
          this.loading=false
          console.log(data, 'this is GetDashboardDignosisSpecialityWiseWithRestrictedAccess data');
          ...
          console.log(
            this.ELEMENT_DATA,
            'this is GetDashboardDignosisSpecialityWiseWithRestrictedAccess data'
          );
          ...
          this.dataSource.filterPredicate = ((data, filter) => {
            const a = !filter.occurrence || data.occurrence.toString()  === filter.occurrence;
            ...
            const e = !filter.disease || data.disease.toLowerCase().includes(filter.disease.toLowerCase());
      ...
      // this.Svc_dashboardService.GetDashboardDignosisSpecialityWise(params).subscribe(
      //   (data: any) => {
      //     this.loading=false
      //     console.log(data, 'this is GetDashboardDignosisSpecialityWise data');
      //     this.ELEMENT_DATA = data;
      //     this.HeaderData = data;
      //     this.dataSourceDownload = this.ELEMENT_DATA;
      //     this.isValid=true;
      //     console.log(
      //       this.ELEMENT_DATA,
      //       'this is GetDashboardDignosisSpecialityWise data'
      //     );
      //     this.dataSource = new MatTableDataSource<PeriodicElement>(
      //       this.ELEMENT_DATA
      //     );
  ...
  public Download(value: any) {
    ...
    if (value == 'Excel') {
      ...
      const newArrayOfObj = this.dataSourceDownload.map((item,index) => {
        return {
         'SL No.':item.srNo ,'Cluster':item.cluster,'District':item.districtName,'Block':item.blockName,'PHC':item.phcName,'Phase':item.phase,'Disease':item.disease,'Occurrence':item.occurrence
        };
      });
