...
@Component({
  selector: 'app-maindashboard-diagnosticprescribedtestwise',
  templateUrl: './maindashboard-diagnosticprescribedtestwise.component.html',
  styleUrls: ['./maindashboard-diagnosticprescribedtestwise.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class MaindashboardDiagnosticprescribedtestwiseComponent implements AfterViewInit {
  loading = false;
  ...
  constructor(
    ...
    this.ReportName = 'Diagnostic Prescribed Test wise';
    ...
    // this.GetDashboardDiagnosticPrescribedTestWiseWithRestrictedAccess({
    //   params,
    // });
    // this.GetDashboardDiagnosticPrescribedTestWise({
    //   params,
    // });
    //////////////////////////////////////////////////////////////
    this.dataSource.filterPredicate = ((data, filter) => {
      ...
      const b =
        !filter.diagnostic ||
        data.diagnostic.toLowerCase().includes(filter.diagnostic);
       return a && b;
  ...
  GetDashboardDiagnosticPrescribedTestWiseWithRestrictedAccess(params: any) {
    this.loading=true;
    this.userObjFromToken = this.svcLocalstorage.GetData(
      environment.AuthTokenKeyLSKey
    );
  // GetDashboardDiagnosticPrescribedTestWise(params: any) {
  //   this.loading=true;
  //   this.userObjFromToken = this.svcLocalstorage.GetData(
  //     environment.AuthTokenKeyLSKey
  //   );
    if (this.userObjFromToken) {
      this.Svc_dashboardService.GetDashboardDiagnosticPrescribedTestWiseWithRestrictedAccess(params).subscribe(
        (data: any) => {
          this.loading=false;
          console.log(data, 'this is GetDashboardDiagnosticPrescribedTestWiseWithRestrictedAccess data');
          ...
          console.log(
            this.ELEMENT_DATA,
            'this is GetDashboardDiagnosticPrescribedTestWiseWithRestrictedAccess data'
          );
          ...
          this.dataSource.filterPredicate = ((data, filter) => {
            ...
            const b =
              !filter.diagnostic ||
              data.diagnostic.toLowerCase().includes(filter.diagnostic.toLowerCase());
             return a && b;
          }) as (PeriodicElement, string) => boolean;
        },
        (error)=>{
      ...
      // this.Svc_dashboardService.GetDashboardDiagnosticPrescribedTestWise(params).subscribe(
      //   (data: any) => {
      //     this.loading=false;
      //     console.log(data, 'this is GetDashboardDiagnosticPrescribedTestWise data');
      //     this.ELEMENT_DATA = data;
      //     this.HeaderData = data;
      //     this.dataSourceDownload = this.ELEMENT_DATA;
      //     this.isValid=true;
      //     console.log(
      //       this.ELEMENT_DATA,
      //       'this is GetDashboardDiagnosticPrescribedTestWise data'
      //     );
      //     this.dataSource = new MatTableDataSource<PeriodicElement>(
      //       this.ELEMENT_DATA
      //     );
  ...
  public Download(value: any) {
    ...
    if (value == 'Excel') {
      ...
      const newArrayOfObj = this.dataSourceDownload.map(item => {
        return {
         'SL No.':item.srNo ,'Diagnostic':item.diagnostic,'No. Of Time Prescribed':item.noOfTimePrescribed};
      });
