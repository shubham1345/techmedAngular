...
import { Devices } from '../models/twilio-model.export';
...
export class TwilioDeviceService {
  campermissionName = "camera" as PermissionName;
  ...
  private async isGrantedMediaPermissions() {
    if (navigator && navigator['permissions']) {
      try {
        const result = await navigator['permissions'].query({ name: this.campermissionName });
        if (result) {
